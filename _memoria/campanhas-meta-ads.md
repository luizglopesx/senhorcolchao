# Campanhas Meta Ads — Histórico de Performance
*Senhor Colchão | act_888195439518063 | Atualizado: 20/07/2026*

Referência de benchmarks internos para planejamento e comparação de campanhas futuras.

---

## Histórico de Campanhas

| Campanha | Período | Gasto | CTR | CPC | Conversas (7d) | Custo/conv | Leads nível 5 | Custo/lead quente |
|---|---|---|---|---|---|---|---|---|
| CP04 — Prospect Geral | 02/05–31/05/2026 | R$ 516,79 | 0,88% | R$ 1,66 | 32 | R$ 16,15 | 15 (46,9%) | R$ 34,45 |
| Dia dos Namorados 2026 | 03/06–13/06/2026 | R$ 540,90 | 0,96% | R$ 1,33 | 50 | R$ 10,82 | 33 (66,0%) | R$ 16,39 |
| Aqui da Jogo — Copa 2026 | 15/06–30/06/2026 | R$ 584,18 | 1,31% | R$ 1,07 | 63 | R$ 9,27 | 30 (47,6%) | R$ 19,47 |
| Durma como Campeão — Julho 2026 (CTWA) ✅ encerrada | 06/07–19/07/2026 | R$ 405,82 | 0,62% | R$ 2,11 | 16 | R$ 25,36 | — | — |

> **Durma como Campeão encerrada em 19/07,** conforme `end_time` — confirmado em 20/07 que parou de gastar de verdade (zero impressão no dia 20/07). Fechou pior que as 3 campanhas anteriores em todos os indicadores (custo/conversa R$25,36 vs R$9,27–16,15), mas recuperou nos últimos dias: CTR subiu de 0,56% pra 0,92-1,18% nos dias 17-18/07, puxado pela poda de criativos fracos em 13/07. Frequência final 3,30 — passou do limite de 3,0 nos últimos dias, mas como a campanha já estava terminando não gerou ação.

---

## Acompanhamento — Durma como Campeão (Julho 2026)

- **Checagem em 13/07/2026:** CTR (0,56%) e custo/conversa (R$23,88) piores que as 3 campanhas anteriores registradas abaixo. Frequência (2,84) dentro do ideal — não é fadiga de público.
- **Causa identificada:** performance fraca distribuída entre as 3 artes ativas, não um outlier isolado. Pior caso: **Arte 05 "Férias"** gastou R$15,57 com zero cliques no link e zero conversas.
- **Ação tomada em 13/07/2026:** pausadas **Arte 05 \| Férias** (zero conversão) e **Arte 04 \| Orthoplus** (CPC R$3,40, só 1 conversa). Verba do conjunto concentrada em **Arte 02 \| Orthoplus R$1.890**, que já respondia por 85% do gasto e 7 das 8 conversas.
- **Checagem em 14/07/2026:** pausa de Arte 04/05 confirmada em vigor (ambas `PAUSED`, zero gasto novo desde 13/07). Verba 100% concentrada na Arte 02. Acumulado geral segue fraco (CTR 0,55%, frequência saudável 2,94x, custo/conversa R$25,34 — pior que as 3 campanhas anteriores). Primeiro dia limpo só com Arte 02 (14/07, parcial): CTR subiu pra 0,74% — sinal inicial positivo, mas amostra pequena (1 dia, 4 cliques) pra confirmar tendência.
- **Checagem em 15/07/2026:** tendência diária confirma melhora real desde a poda — CTR subindo dia a dia só com Arte 02 (0,56% média geral → 0,65% em 14/07 → **0,90%** em 15/07 parcial) e CPC caindo (R$2,25 em 14/07 → **R$1,33** em 15/07, perto da meta interna de R$1,20). Frequência subiu pra 3,08x — ainda dentro do ideal (2,5–3,5), mas no limite superior. **Ponto de atenção:** nenhuma conversa nova iniciada desde 12/07 (3 dias parado) — tráfego mais barato e mais clicado, mas não está virando conversa no WhatsApp.
- **Pendente:** se não voltar a puxar conversa até 16/07, considerar trocar a copy/CTA da Arte 02 (frequência subindo pode indicar início de fadiga) ou encerrar a campanha antes do previsto. Fechar os números finais em 19/07 pra registrar no histórico definitivo.
- **Checkpoint em 16/07/2026 (gatilho cumprido):** confirmado — nenhuma conversa nova desde 12/07, 4 dias seguidos (13-16/07) com ~R$104 gastos e zero conversa. Frequência caiu pra 2,15 (saudável, não é fadiga de público — problema é criativo/copy). Decisão apresentada ao usuário: trocar pra Arte 03 (SmartFlex, reserva) nos 3 dias finais da Fase 1, ou deixar rodar até 19/07 já que a Fase 2 entra em 20/07 com criativos novos. Aguardando resposta — ver `2026-07-16-durma-como-campeao-snapshot.md`.
- **Correção em 17/07/2026:** a "estagnação" do checkpoint de 16/07 era atraso de atribuição do Meta, não dado real — 15/07 teve 2 conversas novas e 16/07 teve 1, que não tinham aparecido ainda na checagem do dia anterior. Decisão de deixar rodar até 19/07 sem trocar criativo segue válida (e mais confortável agora). **Lição:** dado de 1-2 dias mais recentes no Meta Ads é provisório — evitar decisão definitiva (trocar criativo, encerrar campanha) só com base nele; esperar fechar. Ver `2026-07-17-durma-como-campeao-snapshot.md`.
- **Fechamento em 20/07/2026:** campanha confirmada parada (zero impressão em 20/07). Números finais: R$405,82 gasto, 16 conversas, R$25,36/conversa, frequência 3,30. 18 e 19/07 mantiveram o CTR alto (0,92-1,18%) mas 19/07 (último dia) teve zero conversa nova apesar do gasto — mesmo assim, não valia a pena agir no último dia. Ver `2026-07-18-durma-como-campeao-snapshot.md` e histórico da tabela acima.
- **Desativada manualmente em 21/07/2026:** o objeto da campanha seguia com status `ACTIVE` mesmo sem gastar desde 19/07 (não tinha sido arquivada pelo Meta). Usuário desativou manualmente. Campanhas ativas na conta agora: só Pré-Festa do Peão (Senhor Colchão) e Venda Lotes de Terreno (negócio à parte, não é da Senhor Colchão).

---

## Benchmarks do Segmento (Móveis & Colchões — Meta Ads Brasil)

| Métrica | Referência |
|---|---|
| CTR | 0,90% – 1,20% |
| CPC | R$ 1,50 – R$ 3,50 |
| CPM | R$ 12 – R$ 22 |
| Custo por conversa | R$ 18 – R$ 35 |
| % conversas → nível 5 | 20% – 35% |
| Custo por lead quente | R$ 60 – R$ 120 |

> As 3 campanhas superam o benchmark em todos os indicadores de funil.

---

## Metas Internas (referência para próximas campanhas)

Baseado no melhor resultado de cada indicador entre as campanhas registradas:

| Métrica | Meta |
|---|---|
| CTR | ≥ 1,20% |
| CPC | ≤ R$ 1,20 |
| Custo por conversa | ≤ R$ 11,00 |
| % conversas → nível 5 | ≥ 45% |
| Custo por lead quente | ≤ R$ 20,00 |

---

## Lições Aprendidas

1. **Motion/vídeo é obrigatório nos patrocinados** — Copa 2026 teve 936 views vs 26 da campanha anterior. CTR 36% acima quando vídeo é o criativo principal.
2. **Datas comemorativas = lead mais quente** — Namorados teve 66% das conversas chegando ao nível 5. Audiência já aquecida para compra.
3. **Campanhas sazonais longas = volume** — Copa gerou mais conversas (63) e menor custo por conversa (R$ 9,27), mas com menor % de profundidade.
4. **Always-on (CP04) tem rendimento inferior** — útil para presença contínua, não para picos de demanda.
5. **Frequência ideal:** manter entre 2,5x e 3,5x para evitar fadiga sem perder eficiência.

---

## Calendário de Campanhas — 2º Semestre 2026

| Período | Campanha | Modelo | Verba sugerida |
|---|---|---|---|
| Julho (mata-mata Copa) | Aqui da Jogo — fase mata-mata | Sazonal + motion | R$ 200 |
| Agosto (09/08) | Dia dos Pais | Comemorativa + motion | R$ 500–600 |
| Setembro | Always-on atualizado | CP revisado + motion | R$ 15–20/dia |
| Outubro–Novembro | Pré-Black Friday + BF | Sazonal longa + urgência | R$ 800–1.000 |
| Dezembro | Natal | Sazonal + motion | R$ 600–800 |

---

*IDs de campanha: CP04 `120213139056180752` | Namorados `120245522359900752` | Copa `120246295253580752`*
*Conta: act_888195439518063 | Fonte: Meta Marketing API v25.0*
