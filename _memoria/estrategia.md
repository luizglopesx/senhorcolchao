...
- **Pré-Festa do Peão 2026** — ATIVA desde 20/07, roda até 09/08 (21 dias, R$600 total / ~R$28,57 dia). Esquenta antecipando a Festa do Peão de Barretos (evento 20-30/08) — mix geral de produto com Solteiro em destaque (motivo real da demanda de agosto, ver `empresa.md`). Planejamento completo em `marketing/campanhas/pre-festa-peao-2026/planejamento-campanha.md`. 3 peças via Higgsfield (feed + story + motion) — Colchão D28 Comfortopedic Solteiro (R$479), Cama Box SmartFlex Casal (R$1.690), Colchão Orthoplus Casal (R$1.390).
  - ✅ Orgânico disparado no horário em 20/07: Instagram (feed+story), Facebook confirmados publicados. **WhatsApp Status do lançamento (D28 Solteiro) falhou** (timeout 30s) — reenviado manualmente em 21/07, confirmado.
  - ✅ Meta Ads (Patrocinado/CTWA) ativado manualmente pelo usuário em 20/07 (09h25): campanha `120247929494160752`, ad set `120247929500490752` (raio 40km Barretos + interesses Rodeio/Agronegócio/Móveis/Decoração/Colchão, R$28,57/dia). Fluxo documentado na memória `meta-ads-ctwa-workflow`.
  - **Achado dias 1-2 (20-21/07):** Meta concentrou 81% da entrega no SmartFlex Casal — D28 Solteiro (ângulo prioritário) e Orthoplus quase sem impressão ainda. Normal de fase de aprendizado com só 1-2 dias de dado — reavaliar em 2-3 dias se persistir. 1 lead identificado (SmartFlex), 0 venda confirmada no Bling até 21/07 (esperado, ticket alto não converte no mesmo dia). Ver `2026-07-21-pre-festa-peao-snapshot.md`.
  - ✅ Banner do site (2560x800) criado em 17/07 com base nas referências antigas da marca (madeira + silhueta de rodeio, paleta atual sem vermelho) — `marketing/campanhas/pre-festa-peao-2026/banner-site/banner-pre-festa-peao.jpg`. Referências salvas em `identidade/referencias-visuais.md` e `identidade/referencia-festa-do-peao/`.
  - ✅ 3 WhatsApp Status antigos da Fase 2 cancelada (21, 23, 25/07) mantidos ativos — checado em 17/07: preços corretos (produtos/tamanhos diferentes da Pré-Festa), não é desatualização.
...
## Nova capacidade — atribuição de venda real (21/07)

Agora dá pra saber se um anúncio gerou **venda de verdade**, não só conversa: skill `/atribuicao-vendas` cruza a mensagem automática do WhatsApp (por anúncio) com conversas no Campaign Manager e pedidos no Bling (via resource novo `bling-contato`, adicionado hoje no `nexus-manager.js` — expõe telefone do contato, que faltava). Testado e funcionando na Pré-Festa do Peão (ver achado acima).

## Campanhas encerradas (referência)

- **Durma como Campeão — Julho 2026** ✅ encerrada 19/07 (confirmado parada em 20/07, desativada manualmente em 21/07). R$405,82 gasto, 16 conversas, R$25,36/conversa, frequência final 3,30 — pior que as 3 campanhas anteriores, mas recuperou nos últimos dias (CTR 0,56%→0,92-1,18% após poda de criativos fracos em 13/07). Histórico completo em `_memoria/campanhas-meta-ads.md`.
...
- **Redesign de páginas de produto no Simplo 7** — layout HTML padronizado definido e aprovado (8 seções, cores da marca — ver memória `pagina-produto-simplo7`). Páginas organizadas por produto em `saidas/produtos/<categoria>/<produto>/`. Lista de já feitos cresceu bastante desde o início (inclui agora Fort Comfort, Golden Spring, Esmeralda, Nashville, Vivere C1885/C1910, entre outros, além dos originais Livorno/Tower/Madrid/Dakota/Lancaster/Cloud/D20/D28/Box/Box Baú) — checar `saidas/produtos/` pra lista atual completa. Skill dedicada `/pagina-cama-box` gera a versão cama box de um produto a partir do colchão avulso já cadastrado, puxando as fotos certas do bucket Supabase. **Pendência encontrada em 20/07:** 23 páginas de cama box (Queen/King/Super King) dizem "01 colchão + 01 base" mas usam fotos de base bipartida — deveriam dizer "01 colchão + 02 base bipartida". Lista completa e correção (troca de texto idêntica nas 23) em `saidas/2026-07-20-auditoria-conteudo-base-bipartida.md`. Pendente também: hospedar as imagens no Simplo 7 e colar o código no catálogo — e reavaliar prioridade conforme a decisão da frente "Site novo" abaixo.
...
- **Checagens de 13 a 21/07: CPA de 14 dias em melhora constante e depois estável** — R$44,13 (15/07) → R$40,03 (16/07) → R$36,69 (17/07) → R$27,22 (18/07) → **R$27,81 (21/07)**, mantendo o melhor patamar da sequência. 12 conversões acumuladas em 14 dias. Pausa de conversão no fim de semana (18-19/07) quebrou em 20/07 — variação normal, não vazamento. Nenhuma outra campanha da conta voltou a gastar (só CP01C ativa).
...
- Auditorias: `marketing/campanhas/relatorios/2026-06-26-google-ads-auditoria.md` → ... → `2026-07-11-google-ads-auditoria.md` (primeira conversão + correção de keyword confirmada) → **`2026-07-21-google-ads-auditoria.md`** (mais recente — CPA estável no melhor patamar)
