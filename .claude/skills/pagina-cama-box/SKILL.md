...
- **Estrutura do layout:** memória `pagina-produto-simplo7` (8 seções, cores, CSS) — se não estiver carregada, seguir o padrão do arquivo de referência `saidas/mockup-produto-livorno-v2.html` e de qualquer `descricao-simplo7-*-conjunto.html` já existente (ex: `saidas/produtos/tower/queen/conjunto/`)
...
7. Avisar qualquer fallback usado (ex: foto de cena de outro tamanho, ausência de foto isolada da base) pra o usuário saber que pode trocar depois se tiver material melhor.
